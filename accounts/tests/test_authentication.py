...
class Authenticate_Test(TestCase):
    ...
    def test_sends_assertion_to_mozzila_with_domain(self, mock_post):
    ...
    def test_returns_none_if_status_not_okay(self, mock_post):
        mock_post.returns_value.json.return_value = {'status': 'not okay!'}
        user = self.backend.authenticate('an assertion')
        self.assertIsNone(user)
